...
from sewar.command_line import main
...
class simple_image_download:
    ...
    def download(self, keywords, limit, extensions={'.jpg', '.png', '.ico', '.gif', '.jpeg'}, main_directory="simple_images/",sub=True):
        keyword_to_search = [str(item).strip() for item in keywords.split(',')]
        ...
        while i < len(keyword_to_search):
            self._create_directories(main_directory, keyword_to_search[i],sub)
            url = 'https://www.google.com/search?q=' + quote(
    ...
    def _create_directories(self, main_directory, name,sub=True):
        if (sub == True):
            name = name.replace(" ", "_")
            try:
